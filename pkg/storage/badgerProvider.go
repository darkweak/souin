				...
				_ = it.Item().Value(func(val []byte) error {
					if res, err := http.ReadResponse(bufio.NewReader(bytes.NewBuffer(val)), req); err == nil {
						...
						if validator.Matched {
							provider.logger.Sugar().Debugf("The key %s matched the current iteration key ETag %s", key, it.Item().Key())
							result = res
						} else {
							provider.logger.Sugar().Debugf("The key %s didn't match the current iteration key ETag %s", key, it.Item().Key())
						}
					} else {
