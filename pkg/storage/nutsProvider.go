				...
				if varyVoter(key, req, string(entry.Key)) {
					if res, err := http.ReadResponse(bufio.NewReader(bytes.NewBuffer(entry.Value)), req); err == nil {
						...
						if validator.Matched {
							provider.logger.Sugar().Debugf("The key %s matched the current iteration key ETag %s", key, string(entry.Key))
							result = res
							return nil
						}

						provider.logger.Sugar().Debugf("The key %s didn't match the current iteration key ETag %s", key, string(entry.Key))
					} else {
