...
func Test_SouinKratosPlugin_FilterHandler(t *testing.T) {
...
func Test_SouinKratosPlugin_NewHTTPCacheFilterHandler(t *testing.T) {
	if NewHTTPCacheFilter(DevDefaultConfiguration) == nil {
		t.Error("The NewHTTPCacheFilter method must return an HTTP Handler.")
	}
}

func Test_SouinKratosPlugin_NewHTTPCacheFilter(t *testing.T) {
	httpcache := NewHTTPCache(DevDefaultConfiguration)
	defer func() {
...
func Test_SouinKratosPlugin_NewHTTPCacheFilter_Excluded(t *testing.T) {
	httpcache := NewHTTPCache(DevDefaultConfiguration)
	defer func() {
		_ = httpcache.Retriever.GetProvider().Reset()
	}()
...
func Test_SouinKratosPlugin_NewHTTPCacheFilter_Mutation(t *testing.T) {
...
func Test_SouinKratosPlugin_NewHTTPCacheFilter_API(t *testing.T) {
	httpcache := NewHTTPCache(DevDefaultConfiguration)
	defer func() {
		_ = httpcache.Retriever.GetProvider().Reset()
	}()
	handler := httpcache.handle(nextFilter)
	req, res, res2 := prepare("/souin-api/souin")
	handler.ServeHTTP(res, req)
...
func Test_SouinGinPlugin_Process_APIHandle(t *testing.T) {
	...
	c, r := kratos.CreateTestContext(res)
	c.Request = req
	r.Use(s.Process())
	r.GET("/not-handled", func(c *gin.Context) {
		c.String(http.StatusOK, "Hello, World!")
